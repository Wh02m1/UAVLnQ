...
| 5  | Forced Return-to-Launch (RTL)             | `CreateForcedReturnHomePacket`     | `ExecuteForceRTLAttack`                  | Sends a `SET_MODE` MAVLink packet to force drones into RTL mode. Causes premature mission abortion and retreat to home. |
| 6  | Forced Disarm                             | `CreateForcedDisarmPacket`         | `ExecuteForceDisarmAttack`               | Sends `MAV_CMD_COMPONENT_ARM_DISARM` with the force disarm magic number. Immediately disarms motors mid-flight. |
| 7  | Flight Termination                        | `CreateFlightTerminationPacket`    | `ExecuteFlightTerminationAttack`         | Sends `MAV_CMD_DO_FLIGHTTERMINATION`. Causes drones to immediately cut off motors. |
| 8  | Home Position Hijack                      | `CreateSetHomePositionPacket`      | `ExecuteSetHomeAttack`                   | Maliciously sets a drone's home location near the attacker. On RTL, drones return to attacker's position. |
| 9  | Heartbeat Flood DoS                       | `CreateRealisticHeartbeatPacket`   | `ExecuteHeartbeatFloodAttack`            | Floods the network with MAVLink `HEARTBEAT` messages from multiple spoofed drones (IDs 4–20). Overloads communication and processing. |
